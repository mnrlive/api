const SHEET = "1VIgWeByW0t2CEQA5Sssrze48V_R6wemDV6ws7S-FSgc";
const SHEET_RESOURCES = "1VIgWeByW0t2CEQA5Sssrze48V_R6wemDV6ws7S-FSgc";
const SHEET_RESOURCES_SHEET = "oknbjsw";
const SHEET_RAW_DATA = "oknbjsw";
const SHEET_STATEWISE_TAB = "olt96vg";
const SHEET_CASES_TIME_SERIES_TAB = "o6emnqt";
const SHEET_KEY_VALUES_TAB = "owlnkho";
const SHEET_Tested_Numbers_ICMR_Data = "ozg9iqq";
const SHEET_StateWise_Tested_Numbers_Data = "oknbjsw"
const SHEET_FAQ = "oknbjsw1";
const SHEET_IPTV = "olt96vg";
const SHEET_NAME_FACTOIDS = "oknbjsw";
const SHEET_TRAVEL_HISTORY = "olt96vg";
const SHEET_DATE_WISE_DELTA = "on2tlaw";


const DIR = "./";

const FILE_RAW_DATA = "/resources.json";
const FILE_DATA = "/data.json";
const FILE_FAQ = "/faq.json";
const FILE_WEBSITE_DATA = "/website_data.json";
const FILE_WEBSITE_IPTV = "/websiteiptv.json";
const FILE_TRAVEL_HISTORY = "/travel_history.json";
const FILE_DATE_WISE_DELTA = "/states_daily.json";
const FILE_STATEWISE_TESTED_DATA = "/state_test_data.json";
const FILE_RESOURCES_ESSENTIALS = "/s/resources.json";

module.exports = {
    SHEET,
    SHEET_RESOURCES,
    SHEET_RAW_DATA,
    SHEET_STATEWISE_TAB,
    SHEET_CASES_TIME_SERIES_TAB,
    SHEET_KEY_VALUES_TAB,
    SHEET_Tested_Numbers_ICMR_Data,
    SHEET_FAQ,
    SHEET_IPTV,
    SHEET_NAME_FACTOIDS,
    SHEET_TRAVEL_HISTORY,
    SHEET_DATE_WISE_DELTA,
    SHEET_StateWise_Tested_Numbers_Data,
    DIR,
    FILE_RAW_DATA,
    SHEET_RESOURCES_SHEET,
    FILE_DATA,
    FILE_FAQ,
    FILE_WEBSITE_DATA,
    FILE_WEBSITE_IPTV,
    FILE_TRAVEL_HISTORY,
    FILE_DATE_WISE_DELTA,
    FILE_STATEWISE_TESTED_DATA,
    FILE_RESOURCES_ESSENTIALS,
};
